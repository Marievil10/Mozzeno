...
from functions import get_max_mozzeno_file, determine_filedate, first_weekday, run_everything
    ...
    while True:
        if first_weekday():
            if filedate < cell3:
                ...
                redo_update = str(int('You have already updated your sheet today. Are you you want to do it again? Y/N')).strip().upper()
                if redo_update == 'Y':
                    run_everything()
                else:
